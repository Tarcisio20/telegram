...
const verificarUsuario = (ctx, next) => {
  const mesmoIDMsg = ctx.update.message && ctx.update.message.from.id === env.id

  const mesmoIDCallback = ctx.update.callback_query && ctx.update.callback_query.from.id === env.id
...
bot.start(verificarUsuario, async ctx => {
  ...
  await ctx.reply('Escreva o que deseja incluir na lista....')
  ctx.session.lista = []
})
